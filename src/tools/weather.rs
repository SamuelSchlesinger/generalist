    ...
    async fn execute(&self, input: Value) -> Result<String> {
        let city = input
            ...
            .ok_or_else(|| Error::Other(
                "Missing 'city' field. Example: {\"city\": \"London\"}".to_string()
            ))?;
        ...
        let geocoding_response = client.get(&geocoding_url)
            .send()
            .await
            .map_err(|e| Error::Other(format!("Failed to fetch geocoding data: {}", e)))?;
        
        let geocoding_data: Value = geocoding_response.json()
            .await
            .map_err(|e| Error::Other(format!("Failed to parse geocoding response: {}", e)))?;
        
        let results = geocoding_data.get("results")
        ...
        let lon = location.get("longitude")
            .and_then(|v| v.as_f64())
            .ok_or_else(|| Error::Other("Invalid longitude".to_string()))?;
        let location_name = location.get("name")
            .and_then(|v| v.as_str())
            .unwrap_or(city);
        let country = location.get("country")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        ...
        let weather_data: Value = weather_response.json()
            .await
            .map_err(|e| Error::Other(format!("Failed to parse weather response: {}", e)))?;
        
        let current = weather_data.get("current")
